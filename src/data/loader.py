...
from .dataset import StreamDataset, Dataset, ParallelDataset, CodeswitchDataset
...
def load_cs_data(params, data):
    """
    Load parallel data.
    """
    data['cs'] = {}

    required_cs_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps)

    for src, tgt in params.para_dataset.keys():

        logger.info('============ Codeswitch data (%s-%s-%s)' % (src, tgt, 'cs'))

        assert (src, tgt, 'cs') not in data['cs']
        data['cs'][(src, tgt, 'cs')] = {}
        ...
        for splt in ['train']:
            ...
            # for back-translation, we can't load training data
            if splt == 'train' and (src, tgt) not in required_cs_train and (tgt, src) not in required_cs_train:
                continue

            # load binarized datasets
            src_path, tgt_path, cs_path = params.cs_dataset[(src, tgt, 'cs')][splt]
            
            src_data = load_binarized(src_path, params)
            tgt_data = load_binarized(tgt_path, params)
            cs_data = load_binarized(cs_path, params)

            # update dictionary parameters
            set_dico_parameters(params, data, src_data['dico'])
            set_dico_parameters(params, data, tgt_data['dico'])
            set_dico_parameters(params, data, cs_data['dico'])

            # create ParallelDataset
            dataset = CodeswitchDataset(
                src_data['sentences'], src_data['positions'],
                tgt_data['sentences'], tgt_data['positions'],
                cs_data['sentences'],  cs_data['positions'],
                params
            )
            ...
            data['cs'][(src, tgt, 'cs')][splt] = dataset
            logger.info("")
...
def check_data_params(params):
    ...
    params.cs_dataset = {
        (src, tgt, 'cs'): {
            splt: (os.path.join(params.data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, src)),
                   os.path.join(params.data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, tgt)),
                   os.path.join(params.data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, 'cs')),
                   )
            for splt in ['train']
            if splt != 'train' or (src, tgt) in required_para_train or (tgt, src) in required_para_train
        } for src in params.langs for tgt in params.langs
...
def load_data(params):
    ...
    for (src, tgt, cs), v in data['cs'].items():
        for data_set in v.keys():
            logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.format('Codeswitch data', data_set, '%s-%s' % (src, tgt), len(v[data_set])))
